...
export async function POST(request: Request) {
  ...
  const { action, topic, llmApiKey, llmProvider, bochaApiKey, subtopics } = body;

  if (!topic || !llmApiKey || !llmProvider || !bochaApiKey) {
    return Response.json({ error: "缺少必要参数" }, { status: 400 });
  }

  const config: ResearchConfig = { topic, llmApiKey, llmProvider, bochaApiKey };
  const controller = new AbortController();

  const { stream } = action === "plan"
    ? await runResearchPlan(config, controller.signal)
    : await runResearch(config, controller.signal, subtopics as SubtopicEntry[]);
